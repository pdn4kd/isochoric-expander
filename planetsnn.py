...
for i in np.arange(1,len(stars)):
	if (star_name != stars[i]["HIPnumber"]):
		...
		star_config.write("# Features: Short period (t<"+str(max_period)+" days) planets fit via period, tc, k; e fixed to 0; trends fit\n")
		...
		for x in np.arange(0, planets):
			if (stars["per"][i-planets+x] < max_period):
				...
				star_config.write("anybasis_params['e"+str(short_planets)+"'] = radvel.Parameter(value=0.0)\n")
